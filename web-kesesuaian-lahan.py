...
if not os.path.exists(DATA_DIR):
    st.error("Folder 'data' tidak ditemukan. Pastikan folder data ada di repositori.")
    st.stop()
...
def show_csv():
    ...
    selected = st.selectbox("Pilih file CSV", csv_files, key="csv_select")
...
def show_shp():
    ...
    selected = st.selectbox("Pilih file SHP", shp_files, key="shp_select")
    with st.spinner("Memuat data Shapefile..."):
        try:
...
def show_tif():
    ...
    selected = st.selectbox("Pilih file GeoTIFF", tif_files, key="tif_select")
    with st.spinner("Memuat data GeoTIFF..."):
        try:
